---
id: dicionario-fai
title: Modelo de Informação de Atendimento Individual
order: 5
...
**Regra:** Somente as CBOs apresentadas na :link[Tabela 3 - CBOs que podem registrar o Modelo de Informação de Atendimento Individual]{id=grupo_cbo anchor=modelo-de-informa-o-de-atendimento-individual} podem ser adicionadas no campo CBO do profissional principal.
...
**Referência:** :link[AleitamentoMaterno]{id=dicionario anchor=aleitamentomaterno}.
...
- Não pode ser superior à :link[dataAtendimento]{id=headerTransport anchor=5-dataatendimento}.
...
**Referência:** :link[eMulti]{id=dicionario anchor=emulti}.
...
**Referência:** :link[CondutaEncaminhamento]{id=dicionario anchor=condutaencaminhamento}.
...
**Referência:** :link[racionalidadeSaude]{id=dicionario anchor=racionalidadesaude}.
...
- Todos os encaminhamentos devem ser preenchidos ou com CID10 ou com CIAP2;
...
- Não pode ser preenchido se o campo [stNaoPossuiCpf](#33-stnaopossuicpf) = `false`;
- É obrigatório se o campo [stNaoPossuiCpf](#33-stnaopossuicpf) = `true`.
...
- Deve conter somente CIAP2 que possuem relação com uma CID10, conforme listado na planilha :link[CIAP2 x CID10 mais frequentes]{id=relacao_ciap_cid};
