---
id: dicionario-fvd
title: Modelo de Informação de Visita Domiciliar e Territorial
order: 9
...
**Regra:** Somente as CBOs apresentadas na :link[Tabela 7 - CBOs que podem registrar o Modelo de Informação de Visita Domiciliar e Territorial]{id=grupo_cbo anchor=modelo-de-informa-o-de-visita-domiciliar-e-territorial} podem ser adicionadas no campo CBO do profissional.

**Referência:** :link[UnicaLotacaoHeader]{id=headerTransport anchor=unicalotacaoheader}.

#### \#4 visitasDomiciliares
Lista de registros de Visita Domiciliar e Territorial.
...
* Não pode ser posterior à :link[dataAtendimento]{id=headerTransport anchor=5-dataatendimento} e anterior a 130 anos a partir da :link[dataAtendimento]{id=headerTransport anchor=5-dataatendimento};
* Não deve ser preenchido se o campo [tipoDeImovel](#11-tipodeimovel) = `02` comércio, `03` terreno baldio, `04` Ponto Estratégico, `05` Escola, `06` Creche, `12` Estabelecimento religioso ou `13` CASAI;
...
**Referência:** :link[MotivoVisita]{id=dicionario anchor=motivovisita}.
...
**Referência:** :link[Desfecho]{id=dicionario anchor=desfecho}.
...
**Referência:** :link[tipoDeImovel]{id=dicionario anchor=tipodeimovel}.
...
Código UUID Origem do registro de cadastro domiciliar onde a visita foi realizada.

| Tipo | Obrigatório | Mínimo | Máximo |
| ---- | ----------- | ------ | ------ |
| String | Não | 36 | 44 |

**Regra:** Quando preenchido, deverá ser informado o valor do campo :link[UUID origem do registro de cadastro domiciliar]{id=dicionario-fcd anchor=11-uuidfichaoriginadora} pertencente à :link{id=dicionario-fcd}.
...
- Não pode ser preenchido se o campo [stNaoPossuiCpf](#24-stnaopossuicpf) = `false`;
- É obrigatório se o campo [stNaoPossuiCpf](#24-stnaopossuicpf) = `true`;
- Não é validado se o campo [stNaoPossuiCpf](#24-stnaopossuicpf) = `null`.
