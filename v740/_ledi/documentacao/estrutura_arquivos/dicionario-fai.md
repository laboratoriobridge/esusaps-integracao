...
- Não pode ser preenchido se o campo [cpfCidadao](#24-cpfcidadao) for preenchido.
...
**Referência:** [profissionalFinalizadorObservacao](#profissionalfinalizadorobservacao).
...
**Referência:** [eMulti]({% url dicionario %}#emulti).
...
- Não pode ser posterior à [dataHoraFinalAtendimento](#23-datahorafinalatendimento) e à data atual.
...
- Não pode ser anterior à [dataHoraInicialAtendimento](#22-datahorainicialatendimento);
...
- Ao preencher este grupo, é obrigatório o preenchimento do campo [condutas](#17-condutas) com o valor `4 - Encaminhamento para serviço especializado`.
...
**Referência:** [Tipo de participação no atendimento]({% url dicionario %}#tipoparticipacaoatendimento).
...
{% include ledi_ivcf_campo.md %}

### \#32 solicitacoesOci

{% include ledi_solicitacoes_oci_campo.md %}
...
- Tabela do SIGTAP, competência 11/2025 disponível em: [Tabela Unificada SIGTAP](http://sigtap.datasus.gov.br/tabela-unificada/app/sec/procedimento/publicados/consultar);
...
{% include ledi_solicitacoes_oci_secao.md %}
...
- Não é permitido o preenchimento caso seja informado um Código AB no campo [ciap](#4-ciap);
...
- Pode ser preenchido com Código AB da [ListaCiapCondicaoAvaliada](#listaciapcondicaoavaliada);
...
**Referência:** [SituacaoProblemasCondicoes]({% url dicionario %}#situacaoproblemascondicoes).
...
- Deve ser anterior ou igual [dataAtendimento]({% url headerTransport %}#5-dataatendimento);
- Não é permitido o preenchimento caso seja informado um Código AB no campo [ciap](#4-ciap).
...
- Não pode ser posterior à [dataHoraInicialAtendimento](#22-datahorainicialatendimento);
...
- Se o campo [tipoResultado](#1-tiporesultado) for igual a `1 - Valor` e o valor do campo [exame](#1-exame) for:
	...
	- `0202020509`, então preencher com os valores da tabela [provaLaco]({% url dicionario %}#provalaco);
...
- O campo [profissionalFinalizadorObservacao](#profissionalfinalizadorobservacao) é opcional. Porém se ele for adicionado, é necessário utilizar **true** no campo [ficouEmObservacao](#15-ficouemobservacao).
...
**Referência:** [TipoGlicemiaCapilar]({% url dicionario %}#tipoglicemiacapilar).
