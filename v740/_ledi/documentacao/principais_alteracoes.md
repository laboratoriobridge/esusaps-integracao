...
Foram atualizadas as regras vacinais para aplicação dos imunobiológicos "Pneumo 20 (cod 107)" e "Varicela (cod 34)".
...
Foram incluídos os medicamentos abaixo na {% link catmat %}:
...
- Campo "Especialidade do profissional prescritor" que serve para o usuário informar o CBO do profissional que prescreveu a vacina. Este campo possui uma lista de [CBOs específicos]({% url dicionario %}#cboprescritor);
...
Foi incluída uma nova estratégia chamada "Vacinação escolar" e suas respectivas {% link regras_vacinacao, title:"regras de registro" %}.
...
{% nr %} A API de transmissão de registros no formato LEDI possibilita que sistemas terceiros enviem registros no formato LEDI de forma automatizada. Mais informações sobre como utilizar a API estão na disponíveis na {% link api_transmissao_registro_LEDI, title:"documentação" %}.
...
- As principais regras aplicadas:
    - O campo não pode ser preenchido caso o campo [doseUnica]({% url dicionario-fai %}#4-doseunica) esteja marcado como "Verdadeiro".
